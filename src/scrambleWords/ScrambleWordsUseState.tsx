...
export const ScrambleWords = () => {
  ...
  // Palabra actual sin mezclar
  const [currentWord, setCurrentWord] = useState(words[0]);
  
  // Palabra mezclada
  const [scrambledWord, setScrambledWord] = useState(scrambleWord(currentWord));
  
  // Palabra que la persona escribe
  const [guess, setGuess] = useState(''); 
  
  // Puntos obtenidos
  const [points, setPoints] = useState(0);
  
  // Contador de errores al enviar la palabra actual
  ...
  const handleGuessSubmit = (e: React.FormEvent) => {
    ...
    if (guess === currentWord) {
      ...
      // delete de word guessed from words
      setWords(prev => prev.filter(w => w !== guess));
      //Give Point
      setPoints( (prev) => prev + 1 );
      //Clean up the input (guess = '')
      setGuess('');
      return;
    }
    // ---- user didnt guess the word
  ...
  const handleSkip = () => {
    ...
    //1. add skip point to skip variable
    setSkipCounter( skipCounter + 1 );
    //2. delete the skipped word
  ...
  const handlePlayAgain = () => {
    ...
    //1. define words again
    setWords(newArray);
    //2. remove errores
    setErrorCounter(0);
    //3. remove points
    setPoints(0);
    //4. game over to false again
    setIsGameOver(false);
    //5. skip counter to 0
    setSkipCounter( 0 );
    //6. clean up the input
    setGuess('');
  ...
  useEffect(() => {
    ...
    // is errorCounter === maxAllowErrors ?
    if (errorCounter === maxAllowErrors) {
      setIsGameOver(true);
      return
    }
  }, [words, errorCounter, maxAllowErrors])
